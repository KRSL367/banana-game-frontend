...
import { GamePostHistoryData, useGamePostHistory } from '../hooks/useGamePostHistory';
...
const HomePage: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      setAttempts((prevAttempts) => {
        ...
        return updatedAttempts.length > 3 ? updatedAttempts.slice(-3) : updatedAttempts;
      });
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-gradient-to-br from-yellow-300 to-yellow-600 p-6">
      {/* Game Section */}
      ...
      <div className="w-64 h-[425px] p-4 bg-gray-100 rounded-md overflow-y-auto shadow-lg">
        <h2 className="text-lg font-semibold text-gray-700 mb-2">Game History</h2>
        {attempts.length > 0 ? (
          <ul className="space-y-2">
            {attempts.map((attempt, index) => (
              <li key={index} className="text-gray-600">
                Attempt {index + 1}: {attempt.game_won ? 'Won' : 'Lost'}, Points: {attempt.point_earned}
              </li>
            ))}
          </ul>
        ) : (
